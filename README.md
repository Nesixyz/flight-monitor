...
> 基于飞猪官方 API（flyai-cli），自动监控机票价格，命中低价即推送微信通知 + 在线看板展示。支持去程/回程切换，只需修改 config.json。

**当前监控参数（巴黎 → 上海回程）**：
- 航线：PAR（巴黎）→ SHA（上海）
- 出发日期：2026-10-06 ~ 2026-10-09（共 4 天，全量查询）
- 价格阈值：公务舱 ≤¥13000 / 经济舱 ≤¥5000（含税）
- 中转次数：≤1（直飞优先，接受 1 次中转）
- 推荐航班（推送和看板顶部展示）：公务 ≤¥10000 / 经济 ≤¥5000，日期 10-06 ~ 10-09，总时长最短
...
```jsonc
{
  "origin": "PAR",                         // 出发城市三字码（PAR=巴黎全部机场含CDG戴高乐+ORY奥利+BVA博韦）
  "destination": "SHA",                    // 目的地三字码（SHA=上海全部机场含PVG浦东+SHA虹桥）
  "origin_name": "巴黎",                   // 出发地中文名（推送和看板展示用，缺省自动推断）
  "destination_name": "上海",              // 目的地中文名（同上）
  ...
  "schedule": {
    "phase1_end": "2026-08-25",            // 抓低价期结束日
    "phase2_end": "2026-09-20",            // 稳观察期结束日
    "monitor_end": "2026-10-05"            // 监控停止日（建议设在购票截止后）
  },
  "push": {
  ...
  "recommend": {                           // 推荐航班筛选（可省略，缺省用默认值）
    ...
    "date_start": "2026-10-06",            //   推荐日期窗口起
    "date_end": "2026-10-09"               //   推荐日期窗口止
  }
}
```
...
顶部 **Actions → 左侧选「机票价格监控」 → 右侧 Run workflow**：
- Branch: `main`
